"use client";

import { useEffect, useRef } from "react";
import Image from "next/image";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

export default function NationalLeadership({ executives = [] }) {
  const sectionRef = useRef(null);
  const headingRef = useRef([]);
  const cardsRef = useRef([]);

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.set(headingRef.current, { opacity: 0, y: 24 });
      gsap.set(cardsRef.current, { opacity: 0, y: 30, scale: 0.96 });

      const tl = gsap.timeline({
        defaults: { ease: "power3.out" },
        scrollTrigger: { trigger: sectionRef.current, start: "top 80%" },
      });

      tl.to(headingRef.current, {
        opacity: 1,
        y: 0,
        duration: 0.6,
        stagger: 0.1,
      }).to(
        cardsRef.current,
        {
          opacity: 1,
          y: 0,
          scale: 1,
          duration: 0.5,
          stagger: 0.06,
        },
        "-=0.3"
      );
    }, sectionRef);

    return () => ctx.revert();
  }, []);

  if (executives.length === 0) return null;

  const leader = executives[0];
  const team = executives.slice(1);

  return (
    <section
      ref={sectionRef}
      className="py-12 md:py-16 bg-surface-container-low border-b border-outline-variant/20"
    >
      <div className="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
        {/* Section heading */}
        <div className="text-center mb-10">
          <p
            ref={(el) => (headingRef.current[0] = el)}
            className="font-label-lg text-label-lg text-primary tracking-widest uppercase mb-3"
          >
            Leadership
          </p>
          <h2
            ref={(el) => (headingRef.current[1] = el)}
            className="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-background mb-4"
          >
            National <span className="text-primary">Executives</span>
          </h2>
          <p
            ref={(el) => (headingRef.current[2] = el)}
            className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mx-auto"
          >
            The national executive council oversees the strategic direction of
            the Rural Youth Movement across all 36 states and the FCT.
          </p>
        </div>

        {/* Featured Leader (National Coordinator) */}
        {leader && (
          <div className="mb-16 bg-surface-container-lowest rounded-3xl overflow-hidden border border-primary/20 shadow-[0_8px_30px_rgb(15,122,61,0.08)] flex flex-col md:flex-row relative">
            <div className="absolute top-0 right-0 p-8 opacity-5 text-primary pointer-events-none">
              <span className="material-symbols-outlined text-[180px]">format_quote</span>
            </div>
            
            <div className="w-full md:w-2/5 lg:w-1/3 relative min-h-[300px] md:min-h-0 aspect-[4/5] md:aspect-auto border-b md:border-b-0 md:border-r border-outline-variant/30">
              {leader.photo_url ? (
                <Image
                  src={leader.photo_url}
                  alt={leader.name}
                  fill
                  className="object-cover"
                />
              ) : (
                <div className="w-full h-full bg-gradient-to-br from-primary to-primary-container flex items-center justify-center">
                  <span className="font-headline-lg text-[64px] text-white/50 font-bold">
                    {getInitials(leader.name)}
                  </span>
                </div>
              )}
            </div>

            <div className="w-full md:w-3/5 lg:w-2/3 p-8 md:p-12 flex flex-col justify-center relative z-10 bg-gradient-to-br from-transparent to-primary/5">
              <div className="inline-block bg-primary-container text-white font-bold text-[11px] uppercase tracking-widest px-3 py-1.5 rounded-full mb-4 w-max">
                {leader.role}
              </div>
              <h3 className="font-display-lg text-[32px] md:text-[40px] text-on-background font-bold mb-4 leading-tight">
                {leader.name}
              </h3>
              
              <div className="relative mb-6">
                <p className="font-body-lg text-[16px] md:text-[18px] text-on-surface italic leading-relaxed text-on-surface-variant">
                  "Our mission is to bridge the gap between rural potential and national development. Through grassroots empowerment, we are building a foundation of self-reliance for Nigeria's youth."
                </p>
              </div>

              {leader.bio && (
                <div className="pt-6 border-t border-outline-variant/30 mt-auto">
                  <p className="font-body-sm text-[14px] text-on-surface-variant leading-relaxed">
                    {leader.bio}
                  </p>
                </div>
              )}
            </div>
          </div>
        )}

        {/* Executive cards grid (Rest of the team) */}
        {team.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {team.map((exec, i) => (
              <div
                key={exec.id}
                ref={(el) => (cardsRef.current[i] = el)}
                className="group relative flex flex-col justify-end bg-surface-container-lowest rounded-2xl overflow-hidden aspect-[3/4] md:aspect-[4/5] shadow-[0_4px_20px_rgb(0,0,0,0.05)] hover:shadow-[0_20px_40px_rgb(15,122,61,0.15)] hover:-translate-y-2 transition-all duration-500 cursor-pointer border border-outline-variant/20 hover:border-primary/40"
              >
                {/* Background Photo */}
                <div className="absolute inset-0 w-full h-full">
                  {exec.photo_url ? (
                    <Image
                      src={exec.photo_url}
                      alt={exec.name}
                      fill
                      className="object-cover group-hover:scale-110 transition-transform duration-700 ease-in-out"
                    />
                  ) : (
                    <div className="w-full h-full bg-gradient-to-br from-primary to-primary-container flex items-center justify-center group-hover:scale-110 transition-transform duration-700 ease-in-out">
                      <span className="font-headline-lg text-[40px] text-white/50 font-bold">
                        {getInitials(exec.name)}
                      </span>
                    </div>
                  )}
                </div>

                {/* Gradient Overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-80 group-hover:opacity-100 transition-opacity duration-500" />

                {/* Content */}
                <div className="relative z-10 p-4 md:p-5 text-left transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                  <div className="bg-primary/90 backdrop-blur-md text-surface-white text-[10px] uppercase font-bold tracking-widest px-2 py-1 rounded inline-block mb-2 shadow-sm border border-primary-container/20">
                    {exec.role}
                  </div>
                  <h3 className="font-headline-sm text-[16px] md:text-[18px] text-white font-semibold mb-1 leading-tight">
                    {exec.name}
                  </h3>
                  {exec.bio && (
                    <p className="font-body-sm text-[12px] md:text-[13px] text-white/80 line-clamp-2 md:line-clamp-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                      {exec.bio}
                    </p>
                  )}
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </section>
  );
}

function getInitials(name) {
  return name
    .split(" ")
    .filter(Boolean)
    .map((w) => w[0])
    .slice(0, 2)
    .join("")
    .toUpperCase();
}
